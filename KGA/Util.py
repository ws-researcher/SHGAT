...
# def ra_drop(sparse_index, node_size, dr=0):
#     s_index = tf.squeeze(tf.split(sparse_index, 2, axis=-1)[0])
#     ris = []
#     for i in range(node_size):
#         index = tf.reshape(tf.where(s_index == i), (-1,))
#         ri = [int(index[j]) for j in range(index.shape[0]) if np.random.rand() > dr]
#         if len(ri) == 0:
#             ri = index
#
#         ris.extend(ri)
#
#     sparse_index = sparse_index.numpy()
#     sparse_index = np.array(sorted(sparse_index[ris], key=lambda x: (x[0], x[1])))
#     return sparse_index


# def ra_drop(sparse_index, dr=0):
#     sparse_index = [tf.gather(sparse_index, j).numpy() for j in range(sparse_index.shape[0]) if np.random.rand() > dr]
#     sparse_index = np.array(sorted(sparse_index, key=lambda x: (x[0], x[1])))
#     return sparse_index


def ra_drop(sparse_index, dr=0):

    num = len(sparse_index)
    choice_num = int(num * (1-dr))
    index = list(range(num))
    choice_index = random.sample(index, choice_num)

    sparse_index = tf.gather(sparse_index, choice_index).numpy()
    sparse_index = np.array(sorted(sparse_index, key=lambda x: (x[0], x[1])))
    return sparse_index