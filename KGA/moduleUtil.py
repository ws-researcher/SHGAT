...
def scaled_dot_product_attention1(q, k, mask):
    # query key 相乘获取匹配关系
    matmul_qk = tf.matmul(q, k, transpose_b=True)
...
class MutilHeadAttention1(layers.Layer):
    def __init__(self, input_dim, output_dim, num_heads):
        super(MutilHeadAttention1, self).__init__()
        self.num_heads = num_heads
    ...
    def call(self, k, q, mask=None):
        ...
        # 通过缩放点积注意力层
        attention_weights = scaled_dot_product_attention1(
        q, k, mask)
        # 把多头维度后移
        return attention_weights
